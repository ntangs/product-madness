...
- Settings drawer: provider picker (Google Gemini with free tier, default for fresh saves; or Anthropic Claude), API key (stored only in localStorage), enable toggle, per-provider model picker (Gemini: `gemini-2.5-flash-lite` default / `gemini-2.5-flash`; Claude: `claude-haiku-4-5` default / `claude-sonnet-5`), and "My work context" paragraph feeding the estimator prompt. Legacy saves without a `provider` field resolve to Anthropic.
- Gemini request: `POST https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent` with `x-goog-api-key` header; same system/user prompt text via `system_instruction` + `contents`; `generationConfig` sets `maxOutputTokens: 300`, `responseMimeType: "application/json"`, `thinkingConfig: {thinkingBudget: 0}`. Response text = joined `candidates[0].content.parts[].text`, then the same JSON parse/validation as Claude. Cache keys are provider-scoped (`dayKey|provider|title`).
